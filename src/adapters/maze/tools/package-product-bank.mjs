...
import {
  inspectCssmazeProductBank,
  writeCssmazeProductBankDescriptor,
...
if (!args.source) {
  throw new Error("Usage: package-product-bank.mjs --source <public/cssmaze> [--output <directory>] [--archive <tar.gz>]");
}
const sourceRoot = resolve(args.source);
const outputRoot = resolve(args.output ?? "build/generated/public/cssmaze");
...
try {
  const summary = await inspectCssmazeProductBank(stagingRoot, { verifyDescriptor: false });
  await writeCssmazeProductBankDescriptor(stagingRoot, summary);
  await inspectCssmazeProductBank(stagingRoot);
...
function productPath(url) {
  if (typeof url !== "string" || !url.startsWith("/cssmaze/") || url.includes("..")) {
    throw new Error(`Unsafe cssMaze product URL ${url}`);
  }
...
async function writePortableArchive(productRoot, requestedArchivePath) {
  if (basename(productRoot) !== "cssmaze") {
    throw new Error("Portable cssMaze archives require an output directory named cssmaze");
  }
  ...
  try {
    ...
    if (entries.length === 0 || entries.some(isUnsafeArchiveEntry)) {
      throw new Error("Portable cssMaze archive contains an unsafe or metadata entry");
    }
