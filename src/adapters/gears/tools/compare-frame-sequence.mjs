...
const referenceFrames = join("bench", "results", "cssgears", "reference-frames", "frames");
const browserFrames = join("bench", "results", "cssgears", "browser-frames", "frames");
const reportDir = join("bench", "results", "cssgears", "frame-sequence-compare");
...
if (!report.referenceExists || !report.browserExists) {
  ...
  console.error(JSON.stringify({
    ...report,
    command: "node <frame-sequence-oracle>/scripts/frame-sequence.mjs compare --expected " + referenceFrames + " --actual " + browserFrames + " --out " + reportDir + " --label cssgears-frame-sequence --mean-threshold 1.0 --diff-frames worst",
  }, null, 2));
  process.exitCode = 1;
}
