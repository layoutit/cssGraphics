...
import { mkdir, readFile, readdir, rm, rename, utimes } from "node:fs/promises";
...
if (!args.source || !args.archive) {
  throw new Error(
    "Usage: package-product-bank.mjs --source <public/cssselectropaint> --archive <tar.gz>",
  );
}

const productRoot = resolve(args.source);
if (basename(productRoot) !== "cssselectropaint") {
  throw new Error("ElectroPaint product archives require a directory named cssselectropaint");
}
...
try {
  ...
  if (entries.length === 0 || entries.some(isUnsafeEntry)) {
    throw new Error("ElectroPaint product archive contains an unsafe or metadata entry");
  }
...
async function normalizeArchiveTree(root) {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    else if (!entry.isFile()) throw new Error(`Unsupported ElectroPaint product entry: ${path}`);
