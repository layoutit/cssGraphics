...
test("prepared showreel enters, spins for 15 seconds, and exits without runtime interpolation", () => {
  ...
  assert.equal(showreel.responsivePresentation.schema, "cssgears-responsive-presentation@2");
  assert.equal(showreel.responsivePresentation.breakpointPixels, 600);
  assert.equal(showreel.responsivePresentation.mobile.scaleMode, "prepared-bounds-contain");
  assert.equal(showreel.responsivePresentation.mobile.preparedBounds.schema, "cssgears-prepared-projection-bounds@1");
  assert.equal(showreel.responsivePresentation.mobile.preparedBounds.sourceStateCount, 500);
  assert.ok(showreel.responsivePresentation.mobile.preparedBounds.width > 0);
  assert.ok(showreel.responsivePresentation.mobile.preparedBounds.height > 0);
  assert.equal(showreel.responsivePresentation.mobile.safeInsetPixels, 12);
  assert.equal(showreel.responsivePresentation.mobile.verticalBiasViewportFraction, -0.06);
  assert.equal(showreel.responsivePresentation.runtimeOrientationCalculation, false);
  assert.equal(showreel.responsivePresentation.runtimeGeometryBoundsCalculation, false);
