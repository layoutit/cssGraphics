export async function loadPreparedLightingAsset(lighting) {
  if (lighting?.schema !== "cssgears-prepared-opengl-static-render-atlas@1" ||
      ...
      !Number.isSafeInteger(lighting.width) || !Number.isSafeInteger(lighting.height)) {
    throw new Error("Prepared cssGears lighting asset contract is invalid");
  }
  const response = await fetch(lighting.assetUrl, { cache: "force-cache" });
  if (!response.ok) throw new Error(`Failed to load prepared cssGears lighting: ${response.status}`);
  const bytes = await response.arrayBuffer();
  const actualSha256 = hex(await crypto.subtle.digest("SHA-256", bytes));
  if (actualSha256 !== lighting.assetSha256) {
    throw new Error(`Prepared cssGears lighting hash drifted (${actualSha256})`);
  }
