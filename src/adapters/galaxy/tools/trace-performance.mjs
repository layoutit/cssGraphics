...
const traceProfileId = process.env.CSSGALAXY_TRACE_PROFILE ?? "desktop";
if (traceProfileId !== "desktop" && traceProfileId !== "mobile") {
  throw new Error("Galaxy trace profile is invalid");
}
const traceVariant = process.env.CSSGALAXY_TRACE_VARIANT ?? "translate";
if (!/^[a-z][a-z0-9-]*$/u.test(traceVariant)) throw new Error("Galaxy trace variant is invalid");
const port = 4202;
const baseUrl = `http://127.0.0.1:${port}/`;
const prepared = JSON.parse(await readFile(resolve(repositoryRoot,
  "build/generated/cssgalaxy-product-public/cssgalaxy/prepared.json"), "utf8"));
const profile = prepared.profiles[traceProfileId];
const candidateGalaxyCount = profile.galaxyCount;
const candidateStarCount = profile.starCount;
const candidateSeed = profile.comparisonSeed;
const traceViewport = traceProfileId === "mobile"
  ? { width: 390, height: 844 }
  : { width: 800, height: 600 };
const resultRoot = resolve(repositoryRoot,
  `bench/results/cssgalaxy/performance/${traceProfileId}-g${candidateGalaxyCount}`);
...
const measurementMilliseconds = 56_000;
const encounterSeconds = prepared.encounterReel.encounterFrameCount / prepared.cadence.framesPerSecond;
const expectedEncounterTransitions = Math.floor(measurementMilliseconds / (encounterSeconds * 1_000));
const route = baseUrl;
...
try {
  ...
  await page.addInitScript(() => {
    ...
    window.__cssGalaxyObservedLongTasks = [];
    try {
      const observer = new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          window.__cssGalaxyObservedLongTasks.push({
  ...
  const initial = await page.evaluate(() => {
    performance.mark("cssgalaxy-trace-measurement-start");
    const nodes = Array.from(document.querySelectorAll(".polycss-scene > b"));
    window.__cssGalaxyTraceIdentityNodes = nodes;
    return {
      ...
      inlinePositionCount: nodes.filter((node) =>
        node.style.getPropertyValue("--cssgalaxy-position")).length,
      measurementStartTime: performance.getEntriesByName("cssgalaxy-trace-measurement-start").at(-1).startTime,
    };
  });
  process.stdout.write(`Galaxy trace ready: ${initial.leafCount} retained points; measuring ${measurementMilliseconds} ms\n`);
  await page.waitForTimeout(measurementMilliseconds);
  const final = await page.evaluate(() => {
    performance.mark("cssgalaxy-trace-measurement-end");
    const current = Array.from(document.querySelectorAll(".polycss-scene > b"));
    const retained = window.__cssGalaxyTraceIdentityNodes;
    const marks = performance.getEntriesByType("mark")
      .filter((entry) => entry.name.startsWith("cssgalaxy-"))
    ...
    return {
      ...
      marks,
      longTasks: window.__cssGalaxyObservedLongTasks,
    };
  });
  ...
  const transitionMainThreadTaskMaximumMilliseconds = Math.max(0,
    ...analysis.encounterTransitions.map((transition) => transition.enclosingMainThreadTaskMilliseconds ?? 0));
  ...
  const acceptance = Object.freeze({
    atLeastTwoBankTransitions: final.stats.preparedBankSwitchCount >= 2,
    tracesEveryEncounterReset: final.stats.preparedEncounterSwitchCount >= expectedEncounterTransitions &&
      final.stats.transitions.length === final.stats.preparedEncounterSwitchCount,
    noTransitionMainThreadTaskAbove20Milliseconds: transitionMainThreadTaskMaximumMilliseconds <= 20,
    noTransitionPresentationGapAbove33Milliseconds: transitionPresentationGapMaximumMilliseconds <= 33,
  ...
  const report = Object.freeze({
    schema: "cssgalaxy-headless-performance-trace@1",
    capturedAt: new Date().toISOString(),
    route,
    candidate: Object.freeze({ traceVariant, profileId: traceProfileId,
      galaxyCount: candidateGalaxyCount, starCount: candidateStarCount,
      comparisonSeed: candidateSeed, startupSeed: initial.stats.selectedSeed,
      startupEncounterIndex: initial.stats.startupEncounterIndex,
    ...
    duration: Object.freeze({ requestedMeasurementMilliseconds: measurementMilliseconds,
      ...
      encounterSeconds, coveredEncounterTransitions: final.stats.preparedEncounterSwitchCount,
    ...
    transitions: Object.freeze({ presentationGapMaximumMilliseconds: transitionPresentationGapMaximumMilliseconds,
      ...
      detail: analysis.encounterTransitions }),
    network,
  ...
  console.log(JSON.stringify({ reportPath, tracePath, accepted: report.accepted, acceptance,
    playback: report.playback, transitions: report.transitions, network: report.network,
...
function analyzeNetwork(transfers, readyCdpSeconds, finalCdpSeconds) {
  const normalized = transfers.filter((transfer) => transfer.finishedAtSeconds <= finalCdpSeconds).map((transfer) => {
    ...
    const relativeRoot = `g${candidateGalaxyCount}/${candidateStarCount}`;
    const bank = url.pathname.match(new RegExp(
      `/cssgalaxy/${relativeRoot}/seed-${candidateSeed}/bank-(\\d\\d)-`, "u"));
    return {
      url: transfer.url,
      path: url.pathname,
      type: transfer.type,
...
function analyzeTrace(events, pageMarks) {
  ...
  for (const event of traceMarks) {
    ...
    if (name?.startsWith("cssgalaxy-")) markTimes.set(name, event.ts);
  }
  const startPageMark = pageMarks.find((mark) => mark.name === "cssgalaxy-trace-measurement-start")?.startTime;
  const startTraceMark = markTimes.get("cssgalaxy-trace-measurement-start");
  const encounterPageMarks = pageMarks.filter((mark) =>
    /^cssgalaxy-encounter-transition-\d+-published$/u.test(mark.name));
  const encounterTransitions = encounterPageMarks.map((publishedMark) => {
    const match = publishedMark.name.match(/transition-(\d+)-published/u);
    const ordinal = Number(match[1]);
    const startName = `cssgalaxy-encounter-transition-${ordinal}-start`;
    const stepPageMarks = pageMarks.filter((mark) =>
      new RegExp(`^cssgalaxy-encounter-transition-${ordinal}-step-(\\d+)-published$`, "u")
        .test(mark.name));
...
function traceEventMarkName(event) {
  if (typeof event.name === "string" && event.name.startsWith("cssgalaxy-")) return event.name;
  for (const candidate of [event.args?.data?.name, event.args?.name]) {
    if (typeof candidate === "string" && candidate.startsWith("cssgalaxy-")) return candidate;
  }
  return null;
}
